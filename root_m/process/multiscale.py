...
class Multi(nn.Module):
    def __init__(self):
        ...
        self.conv1 = torch.nn.Conv2d(in_channels=3, out_channels=512, kernel_size=6, padding=1,
                                     stride=16)  # 1st sub sampling
        self.maxpool1 = torch.nn.MaxPool2d(kernel_size=3, stride=2, padding=1)

        self.conv2 = torch.nn.Conv2d(in_channels=3, out_channels=512, kernel_size=6, padding=4,
                                     stride=32)  # 2nd sub sampling
        self.maxpool2 = torch.nn.MaxPool2d(kernel_size=6, stride=1, padding=2)
        
        self.fc = torch.nn.Linear(1536, 3)
